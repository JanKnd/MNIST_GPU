...
fn multiplied_value_transposed_b(start_a : u32, start_b: u32, index: u32) -> f32 { //3 0 0
    var row = index / dims[start_b * 4u + 2u]; //row = index / num_cols_of_b //0
    var col = index % dims[start_b * 4u + 2u]; //col = index % num_cols_of_b //0
    var sum: f32 = 0.0;
    for(var i : u32 = 0u; i < dims[start_a * 4u + 2u]; i = i + 1u) { //for i in range(num_row_of_a)
        var index_a = dims[start_a * 4u] + row * dims[start_a * 4u + 2u] + i; //index in values array for value of a at row and i //8
