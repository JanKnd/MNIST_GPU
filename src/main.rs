...
use crate::run::run_on_example;
use crate::train::run_train;

fn main() {
    env_logger::init();
    let network = network::Network::load("xor.bin");
    //let network = network::Network::new_xor();
    let data = data::Data::new_xor();
    let res = pollster::block_on(run_on_example(network, data));
    res.save("xor.bin");
...
/*
            ...
            pollster::block_on(run::run_on_example(network, data));


            let mut rng = rand::thread_rng();
            //let values: Vec<f32> = (0..7).map(|_| rng.gen_range(-1.0..1.0)).collect();
            let network = network::Network{
                ...
                //values,
                grad: vec![0.; 7],
                dims: vec![0, 2, 1, 1, 2, 1, 2, 1, 4, 1, 1, 1,5,1,1,1,6,1,1,1],
                status: vec![0., 0.1, 0., 0.,0.,],
            };

            let data = data::Data{
                values: vec![1.,1.,0.],
                dims: vec![0, 2, 1, 1, 2, 1, 1, 1],
            };

            pollster::block_on(run_on_example(network, data));
    */
